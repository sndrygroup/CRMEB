	...
	<view class="feedback-wrapper">
		...
		<view class="main">
			...
			<view class="input-box">
				<input type="text" :placeholder="$t(`الرجاء إدخال 姓名`)" v-model="name">
			</view>
			<view class="input-box">
				<input type="text" :placeholder="$t(`الرجاء إدخال 手机号`)" v-model="phone">
			</view>
			<view class="input-box">
				<textarea type="text" :placeholder="$t(`请填写内容`)" v-model="con" />
			</view>
			<view class="sub_btn" @click="subMit">{{$t(`إرسال`)}}</view>
		</view>
	</view>
</template>
			...
			subMit(){
				if(!this.name){
					return this.$util.Tips({
						title: this.$t(`الرجاء إدخال 姓名`)
					})
				}
				if(!this.phone || !(/^1(3|4|5|7|8|9|6)\d{9}$/i.test(this.phone))){
					return this.$util.Tips({
						title:this.$t(`الرجاء إدخال 正确的手机号码`)
					})
				}
				if(!this.con){
					return this.$util.Tips({
